...
<?php
	...
	else
	{
		...
		if($nrows11!=0)
		{
			...
			$sql1="select PerPen from Faculty_staff where SSN='$ssnum'";
			$query1=mysql_query($sql1);
			if(!$query1)
				echo"Failed to search!!!";
			else
			{
				$bsal1="select PerPen from Faculty_staff where SSN='$ssnum'";
		...
		if($nrows2!=0)
		{
			...
			$sql2="select PerPFund from Support_staff where SSN='$ssnum2'";
			$query2=mysql_query($sql2);
			if(!$query2)
				echo"Failed to search!!!";
			else
			{
				$bsal2="select PerPFund from Support_staff where SSN='$ssnum2'";
